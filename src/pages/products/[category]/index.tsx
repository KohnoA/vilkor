...
import { AppRoutes, IconColor } from '@/constants';
    ...
    <Layout title={title}>
      <Texture>
        <section className={`container section ${styles.section}`}>
          ...
          <ul className={styles.list}>
            {testItems.map((product) => {
              const { fraction, volume, price, id } = product;

              return (
                <Card key={id} content={product} link={`${AppRoutes.PRODUCTS_BARK}/${id}`}>
        ...
        <TransitionIcon color={IconColor.WHITE} />
      </Texture>
      ...
      <section className={`container section ${styles.infoSection}`}>
        ...
        <ol className={styles.infoSection__list}>
          ...
          <li className={styles.infoSection__item}>
            <span className={styles.infoSection__num}>🌿8.</span> Кора сосны сохранит влагу, защитит
            корни растений от палящего солнца, сохранит нужную температуру, в случае резких
